...
fetch('data.json').then(r=>r.json()).then(async data=>{
  ...
  const aura = { x: cx, y: cy, vx: (Math.random()*2-1)*AURA_SPEED, vy: (Math.random()*2-1)*AURA_SPEED, r: AURA_RADIUS };
  ...
  function step(){
    ...
    const dx=aura.x-cx, dy=aura.y-cy, dist=Math.hypot(dx,dy)||0.0001;
    const inner = radius - NODE_SIZE*0.6;
    if(dist>inner){
      const nx=dx/dist, ny=dy/dist;
      aura.x = cx + nx*inner; aura.y = cy + ny*inner;
      const dot = aura.vx*nx + aura.vy*ny;
      aura.vx -= 1.8*dot*nx; aura.vy -= 1.8*dot*ny;
    }
  }
